...
# ================================================================
# Usage:
#   ./full_build.sh                    # Full build (all steps)
#   ./full_build.sh --skip-sync        # Skip repo init/sync
#   ./full_build.sh --skip-patches     # Skip patch application
#   ./full_build.sh --gapps-only       # Build only gapps variant
#   ./full_build.sh --vanilla-only     # Build only vanilla variant
#   ./full_build.sh --skip-sign        # Skip signing step
#   ./full_build.sh --skip-upload      # Skip upload step
# ================================================================
...
# Configuration
# ----------------------------------------------------------------
BUILD_ROOT="/aosp/duo-de"
BUILD_DIR="/aosp/duo-de/builds"
SOURCE_DIR="/aosp"
...
if [ "$SKIP_SYNC" = false ]; then
    ...
    while [ $SYNC_RETRY -lt $MAX_SYNC_RETRIES ]; do
        ...
        if repo sync -c --no-clone-bundle --no-tags --optimized-fetch --prune --force-sync -j"$SYNC_JOBS"; then
            log_success "Repo sync completed"
            break
        fi
...
if [ "$SKIP_PATCHES" = false ]; then
    ...
    log_info "Applying ponces personal patches"
    if bash "$PATCHES_DIR/apply-patches.sh" "$(pwd)" ponces; then
        log_success "Ponces patches applied"
    else
        log_warn "Some ponces patches had issues (non-fatal)"
    fi
...
cd "$SOURCE_DIR"
log_success "Treble app built and installed"
